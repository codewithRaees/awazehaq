...
const Header = ({ navLinks }) => {
  ...
  // ✅ Move `donateLink` declaration here (above `return`)
  const donateLink = navLinks.find(link => link.label === "Donate Now");
  console.log(donateLink.path)
    ...
    <div className="w-full shadow-lg sticky top-0 z-10 bg-white">
      <header className="flex justify-between sm:h-24 h-16 items-center px-3">
        <div className="logo flex items-center gap-4 pl-5">
          <img src={logo} className="h-20 w-20 hidden sm:block" alt="Logo" />
          <span className="block text-xl leading-6 text-[#676838]">
            <span className="font-bold">Awaz-e-Haq</span>
            <br />
            Foundation
          </span>
        </div>
        ...
        <div className="header-left flex justify-around items-center sm:gap-5">
          ...
          {/* ✅ Donate Now Button with Dynamic Link */}
          <Link to={donateLink?.path || "#"}>
            <BlinkingButton
              style={{ lineHeight: "23px" }}
              className="bg-[#676838] flex gap-1 px-3 py-1 sm:py-3 font-sans rounded-full sm:font-semibold text-white"
            >
              <span className="text-[14px] sm:text-[16px]">Donate</span>
